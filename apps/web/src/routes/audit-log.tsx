...
import { apiFetchPaged } from '../lib/http';

const PAGE_SIZE = 25;
const AUDIT_VIEW_PERMISSION = 'audit.read';

/** Illustrative, not exhaustive — the actorEmail/action/entityType fields cover free-text search;
 * this dropdown just narrows to modules that actually produce audit entries today (see
 * packages/auth's AUDIT_MODULES, not imported here to avoid pulling apps/api's Node-only MFA
 * primitives into a browser bundle). */
const MODULE_OPTIONS = ['', 'auth', 'rbac', 'users', 'tenants', 'saas', 'security', 'documents', 'notifications', 'platform'];
...
const EMPTY_FILTERS: Filters = { module: '', action: '', entityType: '', actorEmail: '', dateFrom: '', dateTo: '' };
...
export function AuditLogPage() {
  const { tenantSlug, permissions } = useAuth();
  const canView = permissions.includes(AUDIT_VIEW_PERMISSION);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const { data, total: totalCount } = await apiFetchPaged<AuditLogEntryDto[]>(`/tenant/audit-logs?${qs}`, {
  ...
  if (!canView) {
    return (
      <div style={{ maxWidth: 720, margin: '2rem auto' }}>
        <Card>
          <p>You do not have permission to view the audit log ({AUDIT_VIEW_PERMISSION}).</p>
  ...
  return (
    <div style={{ maxWidth: 1100, margin: '2rem auto', display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <h1 style={{ fontSize: '1.25rem' }}>Audit Log</h1>
        <Link to="/dashboard">Back to dashboard</Link>
      </div>
              ...
              <tbody>
                {entries.map((entry) => (
                  <tr key={entry.id} style={{ borderBottom: '1px solid #f3f4f6' }}>
                    ...
                    <td style={{ padding: '6px 8px' }}>{entry.ipAddress ?? '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
